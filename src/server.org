  ...
  get "/upgrades":
    check_token(request):
      pageable:
        ...
        var count: int = 0
        discard countrow[0].parseInt(count)
        let rows = db.getAllRows(sql"select id, type, state, system_board, lock_board, version, url from upgrades where state = ? limit ? offset ?", state, limit, offset)
  ...
  post "/upgrades":
    check_token(request):
      let
        ...
        system = data["system-board"].getNum
        lock = data["lock-board"].getNum
      if db.tryExec(sql"insert into upgrades(type, system_board, lock_board, state, version, url) values(?, ?, ?, 1, ?, ?)", tipe, system, lock, version, url):
...
| name         | type   | default | note                 |
|--------------+--------+---------+----------------------|
| type         | string |   boxos | 升级类型             |
| state        | int    |       0 | 升级状态             |
| system-board | int    |       0 | system 主板型号      |
| lock-board   | int    |       0 | 锁控板型号           |
| version      | int    |      18 | 升级版本             |
| url          | string |         | 升级索引文件链接地址 |
  ...
  put "/upgrades/@id":
    check_token(request):
      let
        ...
        version = data["version"].getNum
        url = data["url"].getStr
      if db.tryExec(sql"update upgrades set type = ?, state = ?, system_board = ?, lock_board = ?, version = ?, url = ? where id = ?", tipe, state, system, lock, version, url, @"id"):
        discard redis.hdel(redisClient, "upgrade.prerelease.${tipe}", "${sytem}-${lock}")
        discard redis.hdel(redisClient, "upgrade.release.${tipe}", "${sytem}-${lock}")
        if state == 1 or state == 2:
          discard redis.hset(redisClient, "upgrade.prerelease.${tipe}", "${sytem}-${lock}", url)
        elif state == 15:
          discard redis.hset(redisClient, "upgrade.release.${tipe}", "${sytem}-${lock}", url)
        resp Http200, @[("content-type", "application/json")], $data
      else:
        resp Http500, "Database Error"
#+end_src
...
#+begin_src nim :noweb-ref delete-upgrade
  delete "/upgrades/@id":
    check_token(request):
      let row = db.getRow(sql"select id, type, system_board, lock_board from upgrades where id = ?", @"id")
      if row[0] != nil:
        let
          tipe = row[1]
          system = row[2]
        ...
        if db.tryExec(sql"delete from upgrades where id = ?", @"id"):
          discard redis.hdel(redisClient, "upgrade.release.${tipe}", "${system}-${lock}")
          discard redis.hdel(redisClient, "upgrade.prerelease.${tipe}", "${system}-${lock}")
          resp Http200, ""
        else:
          resp Http500, "Database Error"
      else:
