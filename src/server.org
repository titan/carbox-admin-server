    ...
    check_token(request):
      pageable:
        if request.headers.has_key("x-method") and $request.headers["x-method"] == "options":
          ...
          let rows = db.getAllRows(sql"select mac, address, system_board, lock_board, wireless, antenna, card_reader, lock_amount, router_board, sim_no from devices limit ? offset ?", limit, offset)
  ...
  get "/upgrades":
    check_token(request):
      pageable:
        ...
        var count: int = 0
        discard countrow[0].parseInt(count)
        let query = "select id, type, state, system_board, lock_board, version, url, \"constraint\", stage from upgrades where state = ? limit ? offset ?"
...
"upgrade.prerelease.${type}" hash 表中，以 "${system}-${lock}-${old-version}" 为
主键保存下载URL 。

代码中用变量 tipe 来升级信息的 type，是因为 "type" 是 nim 语言的关键字，这样可以
避免编译器报错。

stage 是里程碑式的概念，用来表示一个批次的升级信息。在同一个批次中，任何一个已发
布版本的升级都可以升级到最新版本。不同批次之间，老批次的版本只能先升级到新批次的
第一个版本，才能继续往后升级。每当管理员创建一个带有版本约束的升级信息时，stage
自动增加到下一个批次。

增加同一个 stage 的升级信息时，需要同步该 stage 内的各个版本升级链接为最新的升级
链接。增加新 stage 的升级信息时，只能同步上一 stage 中最新版本的升级链接为最新的
升级链接。

如果有未发布的升级信息存在，那么不允许创建新的升级信息。
  ...
  post "/upgrades":
    check_token(request):
      let
        ...
        system = data["system-board"].getNum
        lock = data["lock-board"].getNum
        constraint = data["constraint"].getNum
        maxstagestr = db.getValue(sql"select max(stage) from upgrades where type = ? and system_board = ? and lock_board = ? and state > 0", tipe, system, lock)
      var maxstage: int = 0
      if maxstagestr != nil and maxstagestr != "":
        discard maxstagestr.parseInt(maxstage)
      let newstage: int = if constraint > 0: maxstage + 1 else: maxstage
      let unpublished_count = db.getValue(sql"select count(id) from upgrades where type = ? and system_board = ? and lock_board = ? and state > 0 and state <> 15 and version = (select max(version) from upgrades where type = ? and system_board = ? and lock_board = ? and state > 0 and state <> 15)", tipe, system, lock, tipe, system, lock)
      if unpublished_count != nil and unpublished_count != "0":
        resp Http406, "Found unpublished upgrade"
      else:
        let insertsql = "insert into upgrades(type, system_board, lock_board, state, version, url, \"constraint\", stage) values(?, ?, ?, 1, ?, ?, ?, ?)"
        if db.tryInsertID(sql(insertsql), tipe, system, lock, version, url, constraint, newstage) != -1:
          discard redis.hset(redisClient, "upgrade.prerelease." & tipe, $system & "-" & $lock & "-" & $version, url)
          if constraint > 0:
            let lastversionstr = db.getValue(sql"select max(version) from upgrades where type = ? and system_board = ? and lock_board = ? and stage = ?", tipe, system, lock, maxstage)
            if lastversionstr != nil and lastversionstr != "":
              discard redis.hset(redisClient, "upgrade.prerelease." & tipe, $system & "-" & $lock & "-" & lastversionstr, url)
          else:
            let rows = db.getAllRows(sql"select version from upgrades where type = ? and system_board = ? and lock_board = ? and stage = ? and version < ?", tipe, system, lock, maxstage, version)
            for row in rows:
              discard redis.hset(redisClient, "upgrade.prerelease." & tipe, $system & "-" & $lock & "-" & row[0], url)
          resp Http200, @[("content-type", "application/json")], $data
        else:
          resp Http500, "Database Error"
#+end_src
...
#+begin_src nim :noweb-ref upgrade-detail
  get "/upgrades/@id":
    check_token(request):
      let query = "select id, type, state, system_board, lock_board, version, url, \"constraint\", stage from upgrades where id = ?"
  ...
  put "/upgrades/@id":
    check_token(request):
      ...
      if db.tryExec(sql"update upgrades set state = ? where id = ?", state, @"id"):
        let row = db.getRow(sql"select type, system_board, lock_board, version, stage, url from upgrades where id = ?", @"id")
        if row != nil:
          let
            tipe = row[0]
            system = row[1]
          ...
          if stage != "0":
            var stg: int = 0
            discard stage.parseInt(stg)
            stage = $(stg - 1)
          if state == 15:
            let rows = db.getAllRows(sql"select version from upgrades where system_board = ? and lock_board = ? and stage = ? and state = 15", system, lock, stage)
            for row in rows:
              let
                version1 = row[0]
                oldfield = system & "-" & lock & "-" & version1
              var url = redis.hget(redisClient, prereleasekey, oldfield)
              if url != nil:
                discard redis.hset(redisClient, releasekey, oldfield, url)
          elif state == -1:
            # 1. remove upgrade from prerelease
            # 2. then rollback the url of other prerelease if needed
            discard redis.hdel(redisClient, prereleasekey, field)
            let newurl = db.getValue(sql"select url from upgrades where type = ? and system_board = ? and lock_board = ? and stage = ? and state = 15 and version = (select max(version) from upgrades where type = ? and system_board = ? and lock_board = ? and stage = ? and state = 15)", tipe, system, lock, stage, tipe, system, lock, stage)
            if newurl != nil and newurl != "":
              for field, value in redis.hPairs(redisClient, prereleasekey):
                if value == url:
                  discard redis.hset(redisClient, prereleasekey, field, newurl)
          resp Http200, @[("content-type", "application/json")], $data
        else:
          resp Http404, "Upgrade not found"
      else:
        resp Http500, "Database Error"
#+end_src
** 删除升级
    ...
    check_token(request):
      ...
      if row[0] != nil:
        ...
        if db.tryExec(sql"delete from upgrades where id = ?", @"id"):
          ...
          discard redis.hdel(redisClient, prereleasekey, field)
          for key, value in redis.hPairs(redisClient, releasekey):
            if value == url:
              discard redis.hdel(redisClient, releasekey, key)
          for key, value in redis.hPairs(redisClient, prereleasekey):
            if value == url:
              discard redis.hdel(redisClient, prereleasekey, key)
          resp Http200, ""
        else:
          resp Http500, "Database Error"
      else:
...
获取最新 stage 下的所有升级信息。该列表可以用于提供版本约束的候选列表。

| name   | type   |    default | note |
|--------+--------+------------+------|
| state  | int    |          1 |      |
| type   | string | supervisor |      |
| system | int    |          1 |      |
| lock   | int    |          1 |      |

#+begin_src nim :noweb-ref list-upgrades-of-latest-stage
  get "/upgrades/stages/latest":
    check_token(request):
      pageable:
        var state: int = 15
        var
        ...
        if lock == "":
          lock = "1"
        let countrow = db.getRow(sql"select count(id) from upgrades where state = ? and type = ? and system_board = ? and lock_board = ? and stage = (select max(stage) from upgrades where state = ? and type = ? and system_board = ? and lock_board = ?)", state, tipe, system, lock, state, tipe, system, lock)
        var count: int = 0
        discard countrow[0].parseInt(count)
        let query = "select id, type, state, system_board, lock_board, version, url, \"constraint\", stage from upgrades where state = ? and type = ? and system_board = ? and lock_board = ? and stage = (select max(stage) from upgrades where state = ? and type = ? and system_board = ? and lock_board = ?) limit ? offset ?"
        let rows = db.getAllRows(sql(query), state, tipe, system, lock, state, tipe, system, lock, limit, offset)
        var upgrades: JsonNode = newJArray()
        for row in rows:
          upgrades.add(row2upgrade(row))
        list_response(count, offset, upgrades)
...
#+begin_src nim :noweb-ref row2upgrade
  ...
  proc row2upgrade(row: seq[string]): JsonNode =
    ...
    discard constraintstr.parseInt(constraint)
    discard stagestr.parseInt(stage)
    result = %* { "id": id, "type": tipe, "system-board": system, "lock-board": lock, "state": state, "version": version, "url": url, "constraint": constraint, "stage": stage }
#+end_src
