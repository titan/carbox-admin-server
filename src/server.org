  ...
  get "/upgrades":
    check_token(request):
      pageable:
        ...
        var count: int = 0
        discard countrow[0].parseInt(count)
        let rows = db.getAllRows(sql"select id, type, state, system_board, lock_board, version, url from upgrades where type = ? and state = ? limit ? offset ?", @"type", @"state", limit, offset)
