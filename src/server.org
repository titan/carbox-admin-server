  ...
  post "/upgrades":
    check_token(request):
      let
        ...
        maxstagestr = db.getValue(sql"select max(stage) from upgrades where type = ? and system_board = ? and lock_board = ?", tipe, system, lock)
      var maxstage: int = 0
  ...
  put "/upgrades/@id":
    check_token(request):
      ...
      if db.tryExec(sql"update upgrades set state = ? where id = ?", state, @"id"):
        if state == 15:
          let rows = db.getAllRows(sql"select type, system_board, lock_board, version, stage from upgrades where stage = (select stage from upgrades where id = ?)", @"id")
          var
            stagestr = ""
            tipe = ""
            system = ""
          ...
          for row in rows:
            ...
            system = row[1]
            lock = row[2]
            var url = redis.hget(redisClient, "upgrade.prerelease." & tipe, system & "-" & lock & "-" & version)
            if url != nil:
              discard redis.hset(redisClient, "upgrade.release." & tipe, system & "-" & lock & "-" & version, url)
          var stage: int = 0
          discard stagestr.parseInt(stage)
          let query = "select version from upgrades where tipe = " & tipe & " and system_board = " & system & " and lock_board = " & lock & " and stage = ?"
          let rows1 = db.getAllRows(sql(query), stage)
          for row in rows1:
            let version = row[0]
            var url = redis.hget(redisClient, "upgrade.prerelease." & tipe, system & "-" & lock & "-" & version)
            if url != nil:
